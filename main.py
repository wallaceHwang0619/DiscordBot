...
nailImages = [
  'Nail.jpg', 'Hypers.jpg', 'Nationalism.png', 'Nayeel_poggers.jpg',
  'Pharaoh.png', 'Trash.png', 'sadge.png', 'nail_eating.png', 'A-squab.jpeg',
  'kiss.jpeg'
]
...
def get_rank(username):
  ...
  for items in ranked_stats:
    if items['queueType'] == 'RANKED_SOLO_5x5':
      soloQ += items['tier'] + ' ' + items['rank'] + ' ' + str(
        items['leaguePoints']) + 'LP'

  for items in ranked_stats:
    if items['queueType'] == 'RANKED_FLEX_SR':
      flexQ += items['tier'] + ' ' + items['rank'] + ' ' + str(
        items['leaguePoints']) + 'LP'
  if soloQ == "" and flexQ == "":
...
def get_masteries(username):
  ...
  for i in range(0, 3):
    playerMastery += 'Champion: ' + champ_dict[str(
      masteries[i]['championId'])] + '\nMastery Level: ' + str(
...
async def on_message(message):
  ...
  for words in keyWords:
    if words.find('stephen') != -1 or words.find('steve') != -1 or words.find(
        'yezihang') != -1:
